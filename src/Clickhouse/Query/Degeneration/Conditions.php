...
class Conditions implements Degeneration
{
    ...
    public function process(string $sql)
    {
        ...
        $sql = preg_replace_callback('#\{if(.{1,}?)\s(.+?)}(.+?)\{else}(.+?)\{/if}#sui', function ($matches) use ($markers) {
            return self::__ifsets($matches, $markers, true);
        }, $sql);
        $sql = preg_replace_callback('#\{if(.{1,}?)\s(.+?)}(.+?)\{/if}#sui', function ($matches) use ($markers) {
            return self::__ifsets($matches, $markers, false);
        }, $sql);
